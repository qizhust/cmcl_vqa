...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # assert len(self.transforms) == 1, "Only one transformation is supported!"
        ...
        if len(names) != 0:
            ...
            for i, name in enumerate(names):
                assert os.path.isfile(f"{data_dir}/{name}.arrow"), f"Arrow file {name}.arrow is missing! "
                with pa.memory_map(f"{data_dir}/{name}.arrow", "r") as fd:
                    tables.append(pa.ipc.RecordBatchFileReader(fd).read_all())

                table_names += [name] * len(tables[i])

            self_table = pa.concat_tables(tables, promote=True)
            if text_column_name != "":
                self.all_texts = self_table[text_column_name].to_pandas().tolist()
